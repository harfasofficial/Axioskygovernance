...
class AuditService:
    ...
    Design principles:
    1. Audit writes can run async for latency-sensitive flows, but the system
       also supports an awaited write path for stronger durability.
    2. Never raise exceptions to the caller from the fire-and-forget wrapper.
    ...
    async def write_durable(self, entry: AuditEntry, payload: dict) -> None:
        """Awaited write path for stronger durability guarantees."""
        await self._write_entry(entry, payload)

    async def log(self, entry: AuditEntry, payload: dict) -> None:
        """
        Best-effort wrapper used by background task scheduling.
        Exceptions are swallowed after logging so they do not crash request flow.
        """
        try:
            await self._write_entry(entry, payload)
